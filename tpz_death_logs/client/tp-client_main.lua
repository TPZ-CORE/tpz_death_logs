...
AddEventHandler("tpz_core:resurrectPlayer", function()
    TRIGGERED = false
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsEntityDead(player) then
            -- Get killer info
            local pedKiller = GetPedSourceOfDeath(player)
            local deathHash = GetPedCauseOfDeath(player)
            local killedByWeapon = WeaponNames[tostring(deathHash)]
            local killedByPlayer = nil
            
            if IsEntityAPed(pedKiller) and IsPedAPlayer(pedKiller) then
                killedByPlayer = NetworkGetPlayerIndexFromPed(pedKiller)
            ...
            elseif IsEntityAVehicle(pedKiller) then
                ...
                if IsEntityAPed(driver) and IsPedAPlayer(driver) then
                    killedByPlayer = NetworkGetPlayerIndexFromPed(driver)
                end
            end


            -- Determine reason
            local deathReason = "DIED"
            
            if killedByPlayer == PlayerId() then

                deathReason = "SUICIDE"
                
            elseif killedByPlayer ~= nil then
                deathReason = "KILLED"
                
            else
                -- Not killed by another player → environment death
